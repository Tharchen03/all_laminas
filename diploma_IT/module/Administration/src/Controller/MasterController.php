	...
	*/
	...
	public function addlocationAction()
	{
		...
		if($this->getRequest()->isPost()){
			...
			$data = array(
					...
					'location_dzo'       => $form['location_dzo'],
					'organization_level'=>$from['organization_level'],
					'prefix'         => $form['prefix'],
		...
		$ViewModel = new ViewModel(array(
				...
				'organizationlevel'     => $this->getDefinedTable(Administration\OrganizationlevelTable::class)->get(array('status'=>'1')),
	...
	public function editlocationAction()
	{
		...
		$ViewModel = new ViewModel(array(
				...
				'organizationlevels'=> $this->getDefinedTable(Administration\OrganizationlevelTable::class)->get(array('status'=>'1')),
			));		 
		$ViewModel->setTerminal(True);
		return $ViewModel;	
	}
	 ...
	 */
	public function organizationlevelAction()
	{
		$this->init();
		$organizationlevelTable = $this->getDefinedTable(Administration\OrganizationlevelTable::class)->getAll();
		$paginator = new \Laminas\Paginator\Paginator(new \Laminas\Paginator\Adapter\ArrayAdapter($organizationlevelTable));
	 ...
	 */
    public function addorganizationlevelAction()
    {
		$this->init();
		$page = $this->_id;
		if($this->getRequest()->isPost()){
			...
			$result = $this->getDefinedTable(Administration\OrganizationlevelTable::class)->save($data);
			if($result):
				$this->flashMessenger()->addMessage("success^ Successfully added new Organization level."); 	             
			else:
				$this->flashMessenger()->addMessage("error^ Failed to add new Organization level.");	 	             
			endif;
			return $this->redirect()->toRoute('setmaster/paginator', array('action' => 'organizationlevel', 'page'=>$this->_id, 'id'=>'0'));
		}
		$ViewModel = new ViewModel([
			'title'        => 'Add New Organization Level',
			'page'         => $page,
		]); 
		$ViewModel->setTerminal(True);
		return $ViewModel;
	...
	/**
	 *  editorganzation Action
	 **/
	public function editorganizationlevelAction()
	{
	    $this->init();
		$id = $this->_id;
		$array_id = explode("_", $id);
		$organizationlevel_id = $array_id[0];
		$page = (sizeof($array_id)>1)?$array_id[1]:'';
		if($this->getRequest()->isPost()){
			$form = $this->getRequest()->getPost();
			$data = array(
				'id'             => $form['organizationlevel_id'],
				'organization_level'  => $form['organization_level'],
				'status'         => $form['status'],
				'author'         => $this->_author,
				'modified'       => $this->_modified
			...
			$result = $this->getDefinedTable(Administration\OrganizationlevelTable::class)->save($data);
			if($result):
				$this->flashMessenger()->addMessage("success^ Successfully edited Organization level."); 	             
			else:
				$this->flashMessenger()->addMessage("error^ Failed to edit organization level.");	 	             
			endif;
			return $this->redirect()->toRoute('setmaster/paginator',array('action'=>'organizationlevel','page'=>$this->_id, 'id'=>'0'));
		}
		$ViewModel = new ViewModel(array(
				...
				'page'          => $page,
				'organizationlevels' => $this->getDefinedTable(Administration\OrganizationlevelTable::class)->get($organizationlevel_id),
		));		 
		$ViewModel->setTerminal(True);
		return $ViewModel;	
	}
