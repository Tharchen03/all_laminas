...
class QualificationTable extends AbstractTableGateway 
{
	protected $table = 'hr_qualification'; //tablename
	...
	{  
	    ...
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from(array('q' => $this->table))
	           ->join(array('e' => 'hr_employee'), 'e.id = q.monk_id', array('employee' => 'fullname', 'employee_id' => 'id'));
	...
	*/
	...
	{
		$where = ( is_array($param) )? $param: array('q.id' => $param);
		$adapter = $this->adapter;
