...
{   
   ...
   /**
	...
	public function payregisterAction(){
		$this->init();
		list($region, $location, $department, $year, $month) = explode('&', $this->_id);
	...
	{
		$this->init();
		list($region, $location, $activity, $year, $month) = explode('&', $this->_id);
		$region = ($region == 0)? "":$region;
		$location = ($location == 0)? "":$location;				
	...
	*/
	...
	{
		$this->init();
		list($month, $year, $region, $location)=explode('&', $this->_id);
		if($year == 0):
			$max_year = $this->getDefinedTable(Hr\PayrollTable::class)->getMax('year');
			$max_month = $this->getDefinedTable(Hr\PayrollTable::class)->getMax('month', array('year' => $max_year));
			$year = ($max_month == 12)? $max_year+1 : $max_year;
	...
	**/
	...
	{
		$this->init();
		list($region, $location, $year, $month) = explode('&', $this->_id);
		$region = ($region == 0)? "":$region;
		$location = ($location == 0)? "":$location;	
		if($year == 0):
			$max_year = $this->getDefinedTable(Hr\PayrollTable::class)->getMax('year');
