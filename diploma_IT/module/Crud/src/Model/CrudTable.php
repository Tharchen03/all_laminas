...
class CrudTable extends AbstractTableGateway 
{
	protected $table = 'crd_details'; //tablename
	...
	public function getReport($data)
	{
		...
		if($data['location_type'] != '-1'){
			$select->where(array('location_type'=>$data['location_type']));
		}
	    $selectString = $sql->getSqlStringForSqlObject($select);
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
		return $results;
	...
	/**
	 * For finance accounting- previous get function with join table
	 * @param Int $id
	 * @return Array
	 */
	public function getwithLT($param, $adm_loc=NULL)
	{
